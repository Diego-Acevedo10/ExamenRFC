...
const RFCForm = ({ paterno, materno, nombre, fechaNacimiento }) => {
  ...
  return (
    <div className="container mt-5 p-4 bg-light rounded shadow-sm">
      <h1 className="text-center text-primary">Generador de RFC</h1>
      <form
        onSubmit={(e) => {
          e.preventDefault();
          calcularRfc();
      ...
      {rfc && (
        <div className="mt-3">
          <h3 className="text-success">RFC Generado:</h3>
          <input type="text" className="form-control text-center fw-bold" value={rfc} readOnly />
        </div>
      )}
    </div>
  );
